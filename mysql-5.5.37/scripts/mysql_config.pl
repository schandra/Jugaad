...
my $execdir = '/home/schandra/Jugaad/mysql/bin';
my $bindir  = '/home/schandra/Jugaad/mysql/bin';

# ----------------------------------------------------------------------
# If installed, search for the compiled in directory first (might be "lib64")
# ----------------------------------------------------------------------

my $pkglibdir = fix_path('/home/schandra/Jugaad/mysql/lib',"libmysql/relwithdebinfo",
                         "libmysql/release","libmysql/debug","lib/mysql","lib");

my $pkgincludedir = fix_path('/home/schandra/Jugaad/mysql/include', "include/mysql", "include");
...
my $flags;
$flags->{libs} =
  [@ldflags,@lib_opts,'',' -lpthread -lm -lrt -ldl ','',''];
$flags->{libs_r} =
  [@ldflags,@lib_r_opts,'','',''];
$flags->{embedded_libs} =
  [@ldflags,@lib_e_opts,'','','','',''];
...
$flags->{cflags}  = [@{$flags->{include}},split(" ",' -Wall -O2 -g -DDBUG_OFF')];
